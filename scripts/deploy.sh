...
if [ "$ECS_SERVICE_EXISTS" == "true" ]; then
    ...
    aws ecs update-service --cluster $AWS_ECS_CLUSTER_NAME --service $ECS_SERVICE --task-definition "$ECS_TASK:$TASK_REVISION" --desired-count 1 --region $AWS_DEFAULT_REGION --load-balancers $AWS_LOAD_BALANCER 
